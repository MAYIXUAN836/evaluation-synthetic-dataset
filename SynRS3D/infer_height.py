...
def get_arguments():
    ...
    """
    parser = argparse.ArgumentParser(description="Inference of Height Estimation")
    ...
    parser.add_argument("--restore_from", type=str, default='./pretrain/RS3DAda_vitl_DPT_height.pth',
...
def main():
    ...
    # 通过head_configs指定模型的输出头，包含回归头和分割头，回归头输出高度值，分割头输出地物类别
    head_configs = [
    ...
    pred_all = np.zeros((patch_size+stride*(R-1), patch_size+stride*(C-1)))
    ...
    for r in tqdm(range(R), desc="Row"):
        for c in tqdm(range(C), desc="Column", leave=False):
            ...
            with torch.no_grad():
                outputs = model(input)
                if args.use_tta:
                    pred = outputs.get('regression', None).data.cpu().numpy()
                    pred = ((pred[0, :, :, :] + pred[1, :, :, ::-1] + pred[2, :, ::-1, :] + pred[3, :, ::-1, ::-1])/4).squeeze()
                else:
                    pred = outputs.get('regression', None).data.cpu().numpy().squeeze()
